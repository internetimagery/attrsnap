...
class Table(object):
    def __init__(s, parent, cols, rows):
        """ Initiate a table.
        cols = ["name", "name"]
        rows = func() """
        s.items = []
    ...
    def build(s):
        ...
        if old:
            cmds.deleteUI(old)
        row = cmds.rowsLayout(nc=len(s.cols))
        ...
        for row in sorted(s.items, key=(lambda x: x[s.order]), reverse=s.dir):
            for gui, col in zip(s.rows(row), cols):
...
def main():
    ...
    cmds.showWindow()
    t = Table(p, ["num1", "num2"], lambda x: cmds.intField(v=x[0]), cmds.text(l=x[1]))
    t.items = rows
    t.build()
